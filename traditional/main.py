...
def LoadImage(infilename) :
    ...
    img = img.resize((400,600),Image.LANCZOS).convert('RGB')
    data = np.asarray(img)
    return data
...
def transfer(args):
    try:
        ...
        img_to_save = Image.fromarray(new_img.astype('uint8'), 'RGB')
        img_to_save.save(texture_img_name + "_" + target_img_name + "_b=" + str(args.block_size) + "_o=" + str(args.overlap) + "_a=" + str(args.alpha).replace(".", "_") + "_t=" + str(args.tolerance).replace(".", "_") + ".png")
    except Exception as e:
        print("Error: ", e)
        sys.exit(1)
